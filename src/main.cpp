...
#if defined(NDEBUG)
#error "JebatCoin cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zJCOIN spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zJCOIN spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zJCOIN spend with serial %s from tx %s is not in valid range\n",
    ...
    int64_t nSubsidy = 0;
      ...
      if (nHeight == 0) {
             nSubsidy = 5000000 * COIN;
                ...
                nSubsidy = 1500 * COIN;
      } else if (nHeight >= Params().LAST_POW_BLOCK() && nHeight < 263160) {
                nSubsidy = 5 * COIN;
      } else if (nHeight >= 263160 && nHeight < 526320) {
                nSubsidy = 2.5 * COIN;
      } else if (nHeight >= 526320 && nHeight < 789480) {
                nSubsidy = 1.25 * COIN;
      } else if (nHeight >= 789480 && nHeight < 1052640) {
                nSubsidy = 0.625 * COIN;
      } else if (nHeight >= 1052640 && nHeight < 1315800) {
                nSubsidy = 0.312 * COIN;
      } else if (nHeight >= 1315800 && nHeight < 1578960) {
                nSubsidy = 0.125 * COIN;
      } else if (nHeight >= 1578960 && nHeight < 1842120) {
                nSubsidy = 0.0625 * COIN;
      } else if (nHeight >= 1842120 && nHeight < 2105280) {
                nSubsidy = 0.0312 * COIN;
      } else if (nHeight >= 2105280 && nHeight < 2368440) {
                nSubsidy = 0.01 * COIN;
      } else if (nHeight >= 2368440 && nHeight < 2631600) {
                nSubsidy = 0.005 * COIN;
      } else if (nHeight >= 2631600 && nHeight < 2894760) {
                nSubsidy = 0.0025 * COIN;
      } else if (nHeight >= 2894760 && nHeight < 3157920) {
                nSubsidy = 0.00125 * COIN;
      } else if (nHeight >= 3157920 && nHeight < 3421080) {
                nSubsidy = 0.0006 * COIN;
      } else {
          nSubsidy = 1.25 * COIN;
      }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
      ...
      if (nHeight < 101) {
              ret = blockValue * 0;
      } else if (nHeight >= 101 && nHeight < 1052640) {
              ret = blockValue * 0.8;
      } else if (nHeight >= 2500 && nHeight < 1052640) {
              ret = blockValue * 0.85;
      } else if (nHeight >= 1052640 && nHeight < 2105280) {
              ret = blockValue * 0.90;
      } else {
              ret = blockValue * 0.95;
      }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from JebatCoin
...
{
    RenameThread("jebatcoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZJEBATCOINMinted()
{
...
void RecalculateZJEBATCOINSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
        ...
        //Remove spends from zJCOIN supply
...
bool RecalculateJEBATCOINSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zJCOIN with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zJCOINSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert JEBATCOIN to zJCOIN
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zJCOIN spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zJCOIN serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zJCOIN mints and %d zJCOIN spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // JebatCoin: We use certain sporks during IBD, so check to see if they are
