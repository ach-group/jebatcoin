...
#include "ui_zjebatcoincontroldialog.h"
...
std::list<std::string> ZJebatCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZJebatCoinControlDialog::listMints;

ZJebatCoinControlDialog::ZJebatCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZJebatCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZJebatCoinControlDialog::~ZJebatCoinControlDialog()
{
    delete ui;
}

void ZJebatCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZJebatCoinControlDialog::updateList()
{
...
void ZJebatCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZJebatCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZJebatCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZJebatCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZJebatCoinControlDialog::GetSelectedMints()
{
...
void ZJebatCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
