...
#include "zjebatcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zJCOIN ought to be enough for anybody." - Bill Gates, 2017
    ui->zJCOINpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzJCOINSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzJCOIN->setEnabled(false);
        ui->pushButtonMintzJCOIN->setToolTip(tr("zJCOIN is currently disabled due to maintenance."));

        ui->pushButtonSpendzJCOIN->setEnabled(false);
        ui->pushButtonSpendzJCOIN->setToolTip(tr("zJCOIN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zJCOINpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzJCOIN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zJCOIN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zJCOIN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zJCOIN in ") +
...
void PrivacyDialog::on_pushButtonSpendzJCOIN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zJCOIN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zJCOIN
        sendzJCOIN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zJCOIN
    sendzJCOIN();
}

void PrivacyDialog::on_pushButtonZJebatCoinControl_clicked()
{
    ZJebatCoinControlDialog* zJCOINControl = new ZJebatCoinControlDialog(this);
    zJCOINControl->setModel(walletModel);
    zJCOINControl->exec();
}

void PrivacyDialog::setZJebatCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzJCOINSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzJCOIN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid JebatCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zJCOINpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zJCOIN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " JEBATCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zJCOIN</b>";
    ...
    // use mints from zJCOIN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZJebatCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZJebatCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zjebatcoin selector in case it was used
    ZJebatCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zJCOIN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " JebatCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zJCOIN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zJCOIN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zJCOIN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zJCOIN "));
    ui->labelzJCOINAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzJCOINSyncStatus->setVisible(fShow);
}
